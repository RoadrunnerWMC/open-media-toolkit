...
static inline unsigned long omt_FastMilliseconds(void) 
{
	UnsignedWide	wide;

	Microseconds(&wide);
	return (unsigned long)(((double) WideTo64bit(wide) + 500.0) / 1000.0);
...
{
	return (unsigned long)(omt_FastMilliseconds());
}
...
unsigned long OMediaTimeCounter::get_millisecs(void)
{
	...
	return (unsigned long)(omt_FastMilliseconds())-millisecs_delta;
}
